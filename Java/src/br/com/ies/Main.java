...
public class Main {
	...
	public static void main(String[] args) throws IllegalArgumentException, IllegalAccessException, SQLException {
        ...
        */
		...
		/*
		 * PersistanceParameterDTO<String> persistanceParameterDTO = new
		 * PersistanceParameterDTO<>(); persistanceParameterDTO.setObject(localEntity);
		 * persistanceParameterDTO.setParameter("SELECT * FROM evento.local");
		 * 
		 * PERSISTANCE_MANAGER.getPersistanceList().get(1).select(
		 * persistanceParameterDTO, (object) ->{ List<Object[]> objetos =
		 * (List<Object[]>) object; for(int i =0;i<objetos.size();i++) { for(Object t :
		 * objetos.get(i)) { System.out.println(t); } } });
		 */
	}
